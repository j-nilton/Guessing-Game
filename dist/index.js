...
let pesoFalsa = sortearPesoMoedaFalsa(10); //criei uma função para retornar o peso da moeda falsa. A função precisa receber o peso da verdadeira.
//alterar o peso da moeda falsa
m3 = pesoFalsa; //aqui estamos colocando que a moeda m3 é a falsa
let moedaFalsa = descobrirMoedaFalsa(); //aqui está a função que você precisa criar
...
function procurarMoedaFalsaEntre4(m1, m2, m3, m4, moedaVerdadeira) {
...
function procurarMoedaFalsaEntre2(m1, m2, moedaVerdadeira) {
...
function pesarMoedas(grupo1, grupo2) {
    let g1 = grupo1.split(' ');
    let g2 = grupo2.split(' ');
    let p1 = 0;
    let p2 = 0;
...
function sortearPesoMoedaFalsa(pesoVerdadeira) {
    let novoPeso = pesoVerdadeira + (Math.random() * 2 - 1);
